    ...
    AuthService.login(loginForm)
      .then(response => {
        if (response.data.access_token) {
          ...
          showAlert(`Login failed ! ${response.data.message}`, 'error')
        }
      })
      .catch(error => {
        console.log(error);
    ...
    <section className="m-8 flex gap-4">
      <div className="w-full lg:w-3/5 mt-24">
        <div className="text-center">
          <Typography variant="h2" className="font-bold mb-4" placeholder={''}>Sign In</Typography>
          <Typography variant="paragraph" color="blue-gray" placeholder={''} className="text-lg font-normal">Enter your email and password to Sign In.</Typography>
        </div>
        <form onSubmit={handleLogin} className="mt-8 mb-2 mx-auto w-80 max-w-screen-lg lg:w-1/2">
          <div className="mb-1 flex flex-col gap-6">
            <Typography variant="small" placeholder={''} color="blue-gray" className="-mb-3 font-medium">
              Your email
            </Typography>
            <Input
              size="lg"
          ...
          <Button type="submit" className="mt-6" fullWidth placeholder={''}>
            Sign In
          </Button>
